...
parse(Input) when is_binary(Input) ->
  _ = setup_memo(),
  Result = case 'queries'(Input,{{line,1},{column,1}}) of
             {AST, <<>>, _Index} -> AST;
             Any -> Any
           end,
  release_memo(), Result.

-spec 'queries'(input(), index()) -> parse_result().
'queries'(Input, Index) ->
  p(Input, Index, 'queries', fun(I,D) -> (p_choose([p_zero_or_more(p_seq([fun 'querydef'/2, p_optional(fun 'crlf'/2)])), p_string(<<"">>)]))(I,D) end, fun(Node, _Idx) ->
...
'name'(Input, Index) ->
  p(Input, Index, 'name', fun(I,D) -> (p_seq([fun 'comment_marker'/2, fun 'space'/2, p_string(<<"name:">>), fun 'space'/2, p_label('name', p_one_or_more(p_seq([p_not(fun 'crlf'/2), p_anything()]))), fun 'crlf'/2]))(I,D) end, fun(Node, _Idx) ->binary_to_atom(iolist_to_binary(proplists:get_value(name, Node)), utf8) end).

-spec 'colon_name'(input(), index()) -> parse_result().
'colon_name'(Input, Index) ->
  p(Input, Index, 'colon_name', fun(I,D) -> (p_seq([fun 'comment_marker'/2, fun 'space'/2, p_string(<<":">>), p_label('name', p_one_or_more(p_seq([p_not(fun 'crlf'/2), p_anything()]))), fun 'crlf'/2]))(I,D) end, fun(Node, _Idx) ->binary_to_atom(iolist_to_binary(proplists:get_value(name, Node)), utf8) end).

-spec 'query'(input(), index()) -> parse_result().
'query'(Input, Index) ->
  p(Input, Index, 'query', fun(I,D) -> (p_seq([p_label('chars', fun 'substatement'/2), p_zero_or_more(p_seq([p_label('param', fun 'param'/2), p_label('chars', fun 'substatement'/2)]))]))(I,D) end, fun(Node, _Idx) ->
...
'comment_marker'(Input, Index) ->
  p(Input, Index, 'comment_marker', fun(I,D) -> (p_string(<<"--">>))(I,D) end, fun(Node, _Idx) ->Node end).
